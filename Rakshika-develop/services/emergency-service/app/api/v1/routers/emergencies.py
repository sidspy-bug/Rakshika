...
from ..schemas.emergency import EmergencyRead, EmergencyStatusUpdateRequest, SosTriggerRequest, MeshRelayUploadRequest
...
@router.post("/emergencies/relay-upload", response_model=EmergencyRead, status_code=201)
async def relay_upload(
    payload: MeshRelayUploadRequest,
    principal: Principal = Depends(get_current_principal),
    service=Depends(get_emergency_service),
) -> EmergencyRead:
    controller = EmergencyController(service)
    return await controller.relay_upload(principal, payload)
