"""Rakshika notification service application."""
...
from .api.v1.routers.notification import router as notification_router
from .core.config import get_settings
from .core.database import DatabaseBundle
from .core.event_listener import EmergencyEventListener


@asynccontextmanager
async def lifespan(app: FastAPI):
    """Setup and teardown database, Redis, and event listener resources."""
    settings = get_settings()
    configure_logging(settings.logging)
    logger = logging.getLogger("rakshika.notification")
    
    db = DatabaseBundle.from_settings(settings.database)
    
    # Initialize Redis for event listening
    redis_client = None
    event_listener = None
    try:
        ...
        logger.info("Redis connected for SOS event listening")
        
        # Start the background event listener
        event_listener = EmergencyEventListener(
        ...
        logger.warning("Redis unavailable — SOS event listener disabled: %s", exc)
    ...
    logger.info("Notification service resources initialized")
    try:
        yield
    finally:
        ...
        if redis_client is not None:
            await redis_client.aclose()
        await db.dispose()
        logger.info("Notification service resources released")
