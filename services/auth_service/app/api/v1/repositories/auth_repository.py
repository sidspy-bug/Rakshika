...
class AuthRepository:
    ...
    async def create_user(self, *, user_id: UUID | None = None, full_name: str, email: str, phone: str, password_hash: str, status: str = "active") -> User:
        kwargs = {
            "full_name": full_name,
            "email": email,
            "phone": phone,
    ...
    async def assign_roles(self, user: User, roles: Sequence[Role]) -> None:
        from ..models.auth import user_roles

        values = [{"user_id": user.id, "role_id": role.id} for role in roles]
        if values:
            await self.session.execute(insert(user_roles).values(values))
            await self.session.flush()
