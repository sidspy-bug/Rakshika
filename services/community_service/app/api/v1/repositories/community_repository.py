...
class CommunityRepository:
    ...
    async def list_nearby_available_responders(self, lat: float, lng: float, radius_km: float) -> list[CommunityMember]:
        """Query active available responders within a spatial radial distance in kilometers using PostGIS ST_DistanceSphere."""
        from sqlalchemy import text

        # Convert km radius limit to meters for ST_DistanceSphere checking
        ...
        query = select(CommunityMember).where(
            ...
            text("""
                ST_DistanceSphere(
                    ST_SetSRID(ST_MakePoint(CAST(last_longitude AS DOUBLE PRECISION), CAST(last_latitude AS DOUBLE PRECISION)), 4326),
