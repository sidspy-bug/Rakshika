...
class CommunityService:
    ...
    async def get_nearby_responders(self, lat: float, lng: float, radius_km: float | None = None) -> list[CommunityMember]:
        """Find all available responders within a specific radial distance using PostGIS ST_DistanceSphere."""

        limit_radius = radius_km or self._settings.community.default_radius_km
        return await self._repository.list_nearby_available_responders(lat, lng, limit_radius)

    async def broadcast_emergency(self, request: BroadcastRequest) -> EmergencyBroadcast:
        """Log a broadcast to nearby responders and dispatch FCM push alerts via notification service."""
        ...
        # Trigger push notifications to all discovered nearby responder user device tokens
        recipients_tokens = [f"device-token-{m.user_id}" for m in nearby]
        if recipients_tokens:
            try:
                # Dispatch push alerts to the notification microservice async
                async with httpx.AsyncClient() as client:
                    await client.post(
                        "http://localhost:8006/api/v1/notifications/push",
                        json={
                            "tokens": recipients_tokens,
                            "title": "🚨 Safe Walk Emergency: Help Needed Nearby!",
                            "body": "An emergency alert was broadcast nearby. Accept to see coordinates.",
                            "data": {
                                "emergencyId": str(request.emergency_id),
                ...
                # Log dispatch warning but do not halt database broadcast transactions
                print(f"Failed to post broadcast push notification: {err}")
